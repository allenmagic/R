...
# 设置工作目录
folder_path <- "E:/Documents/r_project/data_source/entity/ROD_Cayman/"

# 获取目录下所有excel文件的列表
file_list_cayman <- list.files(path = folder_path, pattern = "\\.xlsx$", full.names = TRUE)
...
read_and_label <- function(file_path) {
  ...
  date_col_name <- grep("DATE|Date|date", colnames(data), value = TRUE, ignore.case = TRUE, perl = TRUE)
...
combined_df_cayman <- map_dfr(file_list_cayman, read_and_label, .id = NULL) %>%
  select_if(~ !all(is.na(.))) %>%
...
# write_xlsx(
#   combined_df_cayman,
#   '~/Projects/r_project/export_document/entity/fix_issue.xlsx',
#   format_headers = TRUE,
# )
